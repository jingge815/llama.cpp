...
`dpu.h` 运行时把很窄的 F32 `GGML_OP_MUL_MAT` 和 opt-in
`GGML_OP_SCALE` 场景 offload 到 Downmem RV simulator. `MUL_MAT`
覆盖单向量 GEMV, 也覆盖右端输入为少量列时的 batched MUL_MAT.
`SCALE` 主要用于验证后端多算子链路.
...
`GGML_OP_MUL_MAT` 和 `GGML_OP_SCALE`, 并且输出必须是 F32.
...
- `tests/test-downmem-backend.cpp`: 构造小型 F32 `GGML_OP_MUL_MAT` 和
  `GGML_OP_SCALE` 图, 将结果与 CPU 后端比较.
...
| `GGML_DOWNMEM_ENABLE_SCALE` | 未设置 | 启用窄范围 F32 `GGML_OP_SCALE` 支持. |
| `GGML_DOWNMEM_MAX_SCALE_OPS` | `1` | 最多 claim 多少个符合条件的 SCALE op. |
...
现在可在一次 launch 内处理多个 RHS 向量, 并通过内部 opcode 处理 SCALE.

后端还支持一个实验性 F32 `GGML_OP_SCALE` 路径, 需要设置
`GGML_DOWNMEM_ENABLE_SCALE=1`. 该路径只支持线性 F32 输入和输出,
执行 `y = x * scale + bias`, 主要用于验证 Downmem 后端的多算子链路.
...
每个被执行的 SCALE op 会将线性 F32 输入平均分片到多个 DPU. 每个 DPU 的
MRAM 布局如下:
...
host 端写入 `scale` 和 `bias` 参数, DPU 端计算 `y = x * scale + bias`,
然后 host 端读回并写入 ggml `dst`.
...
6. 运行 ggml 层 Downmem 正确性测试, 并要求同时出现
   `test-downmem-backend passed`, `executed op=1`, `m=` 和 `kind=SCALE`.
7. 使用 backend sampling 运行确定性的 CPU completion.
8. 使用 `--device Downmem` 运行同参数的 Downmem completion.
9. 检查 completion offload 日志中同时出现 `MUL_MAT` 和 `kind=SCALE`,
   并比较 CPU stdout 与 Downmem stdout.
...
ggml_downmem_scale_f32: executed op=... kind=SCALE ...
```
...
- `test-downmem-backend`: 构造多个小型 F32 `GGML_OP_MUL_MAT` 图, 覆盖
  `m = 1`, `m = 3` 和 `m = 5`, 并构造多个小型 F32 `GGML_OP_SCALE` 图.
  测试先在 CPU 后端计算, 再在 Downmem 后端计算, 然后用 `1e-4`
  绝对误差或相对误差阈值比较. 如果没有设置 `GGML_DOWNMEM` 或
  `GGML_DOWNMEM_DPU_BIN`, 测试返回 `77`, CTest 会把它视为 skipped.
...
默认 E2E 设置下, completion 阶段会 offload 一个符合条件的 MUL_MAT op,
并通过 backend sampling 触发少量 SCALE op:
...
这样便于检查: completion 输出应与 CPU 一致, 日志中应出现一个
MUL_MAT 执行记录和至少一个 `kind=SCALE` 执行记录. 增大
`GGML_DOWNMEM_MAX_OPS` 可以让更多符合条件的 MUL_MAT op 被 claim.
增大 `GGML_DOWNMEM_MAX_SCALE_OPS` 可以让更多符合条件的 SCALE op 被 claim.
增大或关闭 `GGML_DOWNMEM_MAX_COLS` 可以允许更多 RHS 列, 但当前实现仍然会对
每个 op 经由 host memory staging, 并同步 launch DPU.
...
- 只实现了窄范围 F32 `GGML_OP_MUL_MAT` 和 opt-in `GGML_OP_SCALE`.
...
- `RMS_NORM`, `ADD`, `MUL`, `GLU`, `ROPE` 和 `FLASH_ATTN_EXT` 仍不在这个
  ...
  op 的调度策略. `ADD` 和 `MUL` 可以作为连续 F32 elementwise demo, 但如果
  没有清晰的调度策略, 对当前完整推理链路帮助有限.
